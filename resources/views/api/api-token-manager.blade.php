                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-stone-600">{{ $permission }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                        ...
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center ml-2">
                                    ...
                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="cursor-pointer ml-6 text-sm text-stone-400 underline" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            ...
            <x-input x-ref="plaintextToken" type="text" readonly :value="$plainTextToken"
                class="mt-4 bg-stone-100 px-4 py-2 rounded font-mono text-sm text-stone-500 w-full break-all"
